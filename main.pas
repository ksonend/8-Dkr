...
begin
  ...
  if MessageDlg('Требуется подтверждение',
                'Вы действительно хотите удалить информаию о комиксе "' +
                SG.Cells[0, SG.Row] + '"?',
...
begin
  ...
  try
    ...
    for i:= 1 to SG.RowCount-1 do begin
      ...
      MyCont.Prochit := (SG.Cells[4, i] = 'Да'); //Преобразование в булево значение
      MyCont.avtor:= SG.Cells[5, i];
      //записываем их:
      Write(f, MyCont);
    end;
  finally
...
begin
  ...
  if not FileExists(adres + 'comics.dat') then exit;
  ...
  try
    ...
    while not Eof(f) do begin
      ...
      //добавляем в сетку новую строку, и заполняем её:
        ...
        SG.Cells[4, SG.RowCount-1]:= BoolToStr(MyCont.Prochit, True); //BoolToStr for displaying "Да" or "Нет"
        SG.Cells[5, SG.RowCount-1]:= MyCont.avtor;
    end;
  finally
    CloseFile(f);
  end;
